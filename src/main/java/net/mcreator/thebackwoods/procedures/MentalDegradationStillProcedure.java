...
@EventBusSubscriber
public class MentalDegradationStillProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if ((entity.level().dimension()) == ResourceKey.create(Registries.DIMENSION, ResourceLocation.parse("the_backwoods:the_still"))) {
			entity.getPersistentData().putDouble("still_time", (entity.getPersistentData().getDouble("still_time") + 1));
			if (entity.getPersistentData().getDouble("still_time") >= 24000) {
			...
			} else if (entity.getPersistentData().getDouble("still_time") >= 16800) {
				...
				if (Math.random() < (1) / ((float) 500)) {
					if (world instanceof Level _level) {
						if (!_level.isClientSide()) {
							_level.playSound(null, BlockPos.containing(entity.getX(), entity.getY(), entity.getZ()), BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("the_backwoods:degradation_r1")), SoundSource.MUSIC, (float) 0.67, 1);
						} else {
							_level.playLocalSound((entity.getX()), (entity.getY()), (entity.getZ()), BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("the_backwoods:degradation_r1")), SoundSource.MUSIC, (float) 0.67, 1, false);
						}
					}
				}
			...
			} else if (entity.getPersistentData().getDouble("still_time") >= 9600) {
			...
			} else if (entity.getPersistentData().getDouble("still_time") >= 6000) {
			...
			if (entity.getPersistentData().getDouble("debug_mode") == 1) {
				if (entity instanceof Player _player && !_player.level().isClientSide())
					_player.displayClientMessage(Component.literal(("Still Time: " + entity.getPersistentData().getDouble("still_time"))), true);
			}
		} else {
			{
				TheBackwoodsModVariables.PlayerVariables _vars = entity.getData(TheBackwoodsModVariables.PLAYER_VARIABLES);
			...
			entity.getPersistentData().putDouble("still_time", 0);
		}
	}
}